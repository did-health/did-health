...
export async function decryptFHIRFile({
  encryptedJson,
  litClient,
  sessionSigs,
}: {
...
export async function encryptFHIRFile({
  ...
  try {
    ...
    return {
      encryptedJSON,
      hash: parsed.dataToEncryptHash,
    };
  } catch (err) {
    console.error('Encryption failed:', err);
    throw new Error('Failed to encrypt message: ' + (err instanceof Error ? err.message : String(err)));
  }
}